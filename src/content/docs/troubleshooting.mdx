...
**Check:**
- **Library path is correct and writable** — verify in Settings > General > Library
...
- **Minimum seed time** — this only applies when Delete After Import is enabled, and it gates when the torrent is **removed** from your client after a successful import — it does not delay the import itself. If imports aren't happening, seed time is not the cause. (Settings > General > Import)
- **Import job timing** — Narratorr checks for completed downloads periodically. Give it a minute after the download finishes.
- **Check logs** — set log level to `debug` in Settings > System (Logging section) for detailed import information
...
Narratorr places files into the library folder — it doesn't notify media servers directly. The simplest fix for Audiobookshelf or Plex is a **Connector**: add one in Settings > Connectors and Narratorr pushes a library scan to that server automatically whenever the library changes on import. If you'd rather not use a connector, each server can also detect new books on its own:

- **Audiobookshelf** — add an Audiobookshelf connector (recommended), or rely on its auto-detection of new files in watched folders. If neither, trigger a library scan via the UI or set up a [webhook notification](/configuration/notifications/#webhook) to hit Audiobookshelf's scan API on import.
- **Plex** — add a Plex connector (recommended) to scan on import, or set up periodic scanning in Plex, or trigger it via webhook.
...
**Check:**
- **URL format** — should be `http://flaresolverr:8191` (no `/v1` — Narratorr appends the API path itself)
...
**Check:**
- **ffmpeg not found** — Narratorr auto-detects ffmpeg; there is no path setting. Check the status row in Settings > Audio Tools, and set the `FFMPEG_PATH` environment variable if you need to force a specific binary. In Docker, ffmpeg is pre-installed and detected automatically.
...
- **Event is subscribed** — the notifier must be subscribed to the specific event (grab, download complete, import, failure, health issue, import run finished)
...
- **Script notifier** — ensure the script is executable, exists inside the Docker container (mount as volume), and completes within its configured timeout (default 30 seconds, configurable 1–300s)
...
2. Recreate the container so the new environment applies — a plain `docker compose restart` does not pick up compose-file changes, so the bypass would never activate:
   ```bash
   docker compose up -d narratorr
   ```
...
5. **Remove `AUTH_BYPASS=true`** from your environment and run `docker compose up -d narratorr` again — a plain restart would leave the bypass active
