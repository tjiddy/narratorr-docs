...
If you'd rather not hand-write templates, the **File Naming** section includes a **Preset** dropdown with five built-in format pairs. Selecting one populates both the folder and file format fields; editing either field afterward switches the dropdown to **Custom**.

| Preset | Folder Format | File Format |
|--------|---------------|-------------|
| Standard | `{author}/{title}` | `{author} - {title}` |
| Detailed | `{author}/{series}/{seriesPosition:00? - }{title}` | `{author} - {series? - }{seriesPosition:00? - }{title}{ (?edition?)}{ - ?trackNumber:000}` |
