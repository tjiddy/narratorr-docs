...
These settings all live on the **Search** settings tab, spread across several sections rather than a single Quality page.
...
| **Min Download Size** (MB) | Minimum download size in MB. Filters out tracker-test uploads and single-track previews out of the box. Set to `0` to disable. Default: 50. |
...
| **Minimum Duration** (minutes) | Filters metadata search results (the book candidates shown when searching for or adding a book) shorter than this — useful for dropping promo excerpts and TTS samples. It does not filter indexer release results. Set to `0` to disable. Default: 30. |
...
Configure in **Settings > Search** (When a New Book Is Added section).
...
11. Remaining results are ranked in canonical order: match score, then narrator match (when **Search Priority** is Narrator Accuracy), then quality (MB/hour), with protocol preference, language, indexer priority, grabs, and seeders as successive tiebreakers
...
| **Hold for review** | Narrator mismatch, duration more than 4 minutes off from the existing copy, no quality data to compare, or probe failure | Held in `pending_review` status for your decision |
...
| `duration_delta` | Duration differs from the existing copy by more than 4 minutes (240 seconds) — could indicate an abridged version or wrong content |
| `no_quality_data` | The existing files' quality (MB/hour) couldn't be computed, so there's nothing to compare the new download against. (A true first download — no files on disk — auto-imports without a comparison instead.) |
| `probe_failed` | Couldn't read the audio files to determine quality |
| `unreadable_codec` | Audio files were found, but no readable codec could be determined — unsupported or corrupt format |
