...
Configure remote path mappings per download client in **Settings > Download Clients**. You can add mappings either when creating a new download client or when editing an existing one, in the **Remote Path Mappings** section.
...
When Narratorr receives a path from the download client, it checks if the path starts with any configured remote path. If it matches, the remote prefix is replaced with the local path. If more than one mapping matches, Narratorr uses the longest (most specific) remote path.
...
3. Trailing slashes do not matter — Narratorr normalizes them, so `/downloads` and `/downloads/` behave identically
