...
Configure in **Settings > Post Processing** — the tagging controls are in the Post Processing card at the top of that page. Tagging is disabled by default.
...
Tagging requires **ffmpeg** (pre-installed in Docker). Narratorr detects it automatically — the detection status is shown in **Settings > Audio Tools**, and there is no ffmpeg path setting in the UI. To force a specific binary, set the [`FFMPEG_PATH` environment variable](/configuration/environment-variables/).
...
The extended fields (series, series part, subtitle, ASIN, publisher, description, date, genre) survive MP3 embedding and are picked up by Audiobookshelf, so your media server sees narratorr's metadata rather than the release's. M4B is less forgiving: the container drops series, series part, subtitle, ASIN, and publisher (date, genre, and description survive) — enable the [OPF metadata sidecar](#opf-metadata-sidecar) so Audiobookshelf still sees narratorr's full metadata for M4B files. When writing M4B files, existing **chapter markers are preserved**.
...
Audio processing (merge/convert) is a separate operation, manual by default — the opt-in **Auto-merge multi-file downloads** toggle in **Settings > Post Processing** automates the merge for completed downloads. Either way, tagging does not depend on it.
