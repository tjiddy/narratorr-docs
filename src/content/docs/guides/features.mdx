...
**Active Downloads** — shows all in-progress downloads with real-time progress bars (via SSE streaming). Each card shows download speed, ETA, and progress percentage. Downloads in `pending_review` status show quality gate data with **Approve**, **Reject**, and **Reject & Search** buttons. See [Quality Gates](/guides/quality-gates/) for details.
...
6. Ranks results by quality (MB/hour), with protocol preference as a tiebreaker
