...
:::caution[Pre-1.0 backups can't be restored]
Backups created before the 1.0 migration flatten will fail validation with a "newer version" error and **cannot be restored into 1.0+**. The migration history was reset to a single baseline, so a pre-flatten backup reports more migrations than the current app and is rejected even though it is actually older. This is expected one-time breakage — migrate off pre-flatten versions before relying on backups.
:::
