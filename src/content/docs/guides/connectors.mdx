...
When a book changes in your library, Narratorr fires a refresh at every **enabled** connector. The refresh fires on:

| Trigger | When |
|---------|------|
| **Import** | A book is imported via automatic grab or manual import (copy/move into the library) |
| **Adopt** | A manual import in index-only (in-place) mode registers an existing folder |
