...
Click **Import** to process all selected books. Narratorr moves/copies each book's files into the library using your configured [folder format](/guides/folder-format/), creates database records, and enriches each book's metadata. Tagging and post-processing scripts apply to downloads, not manual imports — manual import does not embed audio tags or run the post-processing script.
