...
| `AUDIBLE_REGION` | `us` | Fallback region for metadata lookups when not configured in Settings > Search (Metadata section). Values: `us`, `ca`, `uk`, `au`, `fr`, `de`, `jp`, `it`, `in`, `es`. |
| `FFMPEG_PATH` | (unset) | Path to an ffmpeg binary, checked ahead of auto-detection (`/usr/bin/ffmpeg`, then the system PATH). If the binary at this path doesn't run, Narratorr falls back to auto-detection. Only needed for manual installs where ffmpeg isn't on the PATH — the Docker image ships ffmpeg. |
| `INSTANCE_BADGE` | (unset) | Free-form badge (e.g. `dev`) for a secondary instance: the client recolors the favicon and prefixes the browser-tab title so the instance is distinguishable from production. Unset or blank = no change. |
