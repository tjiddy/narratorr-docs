...
| Field | Default | Description |
|-------|---------|-------------|
| **Watch Directory** | — | Directory where Narratorr drops `.torrent` or `.nzb` files |
| **Protocol** | `torrent` | Whether Narratorr writes `.torrent` or `.nzb` files (Torrent / Usenet) |

Both fields are required. Narratorr places the release file in the watch directory and immediately marks the download as "completed." It does **not** monitor progress — your external client handles the actual download. Narratorr will attempt to import from the watch folder on the next import cycle.
