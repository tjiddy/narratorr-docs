...
| `library` | object? | Cross-reference: present **only** if this ASIN is already in your library. `{ bookId, status, companionEbook }` — the `bk_` id, its current [status](#book-status-values), and [companion-ebook availability](#companion-ebooks) (`null` when there's nothing to download). |
  ...
  "data": [
    {
      ...
      "library": { "bookId": "bk_3Qp9z...", "status": "imported", "companionEbook": null }
    }
  ],
  "total": 1
}
...
| `companionEbook` | object? | [Companion-ebook availability](#companion-ebooks). Always present — `{ format, sizeBytes }` when an EPUB is available to download, `null` otherwise. |
...
On success, returns `201` with the newly created [book DTO](#get-apiv1booksid). If your Search settings have **search immediately** enabled and the book lands in the `wanted` state, Narratorr fires a release search in the background (the response returns right away regardless).
...
When ebook support is enabled (see [Ebooks settings](/configuration/settings/#ebooks)) and an imported book has an EPUB stored alongside its audio files, the API advertises it and serves the file through a dedicated endpoint. This is the surface [Narratorr Requests](/narratorr-requests/) builds its **Get eBook** feature on.

Availability is advertised via the `companionEbook` field — on the [book DTO](#get-apiv1booksid) and on the metadata-search [`library` cross-reference](#get-apiv1metadatasearch). The field is always present and nullable: `{ format, sizeBytes }` when an EPUB is available to download (`format` is always `"epub"`; `sizeBytes` is the file size in bytes), or `null` whenever there's nothing to download — the feature is off, the book isn't imported, or no usable EPUB is stored with it.

### `GET /api/v1/books/:id/companion-epub`

Streams the book's companion EPUB (`Content-Type: application/epub+zip`, sent as an attachment).

| Status | Body | Meaning |
|--------|------|---------|
| `200` | EPUB bytes | The file. |
| `409` | Error envelope, code `companion_epub_disabled` | Ebook support is disabled. |
| `404` | Error envelope, code `companion_epub_unavailable` | Nothing to serve. Deliberately covers **every** other negative — unknown id, book without an ebook, file missing on disk — the endpoint never distinguishes them. |
| `503` | Error envelope, code `companion_epub_busy` | All download slots are in use. Streams are capped at 4 concurrent; a saturated server answers immediately rather than queueing. Retry shortly. |

The error codes on this endpoint are lowercase (`companion_epub_*`) and stable — branch on the code, not the message.
...
| `progress` | number | Download progress as a fraction from `0` to `1` — multiply by 100 for a percentage. |
...
Feature discovery — ask what this Narratorr supports before relying on it. Returns a single object (not a list):
...
| Field | Type | Notes |
|-------|------|-------|
| `companionEpub.enabled` | boolean | Whether [companion ebook](#companion-ebooks) support is switched on. |

The endpoint fails closed: if the server can't read its settings, it reports `{ "enabled": false }` rather than returning an error.

:::note
Probe **with a valid API key** and branch on the status code. A Narratorr too old to have this endpoint answers `404` — treat that as "unsupported". Authentication runs before routing, so a keyless probe gets `401` on every version; `401` means an auth problem, never "unsupported".
:::
