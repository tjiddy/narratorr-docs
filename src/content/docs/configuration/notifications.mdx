...
| **Import Run Finished** | A Library or Manual import run completes, with queued/held/skipped/failed counts |
...
|----------|-------------|
| `{event}` | The event key (e.g. `on_grab`, `import_run_finished`) |
...
| `{submission.source}` | `library` or `manual` (import-run-finished events only) |
| `{submission.status}` | Import run status (import-run-finished events only) |
| `{submission.counts.accepted}` | Books queued for import (import-run-finished events only) |
| `{submission.counts.held}` | Books held for review (import-run-finished events only) |
| `{submission.counts.skipped}` | Books skipped (import-run-finished events only) |
| `{submission.counts.failed}` | Books that failed (import-run-finished events only) |
