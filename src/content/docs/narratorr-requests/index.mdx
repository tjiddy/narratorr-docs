...
![The narratorr-requests catalog — search the audiobook catalog and hit Request](../../../assets/narratorr-requests/search.png)
...
:::note
You need a **running narratorr instance and an API key** for it (narratorr → Settings →
Security). Until that connection is configured, search returns a *"Narratorr isn't connected
yet"* notice, and approving a request fails with the same message.
:::
...
3. **Search & request.** An approved user searches the catalog and hits **Request** on a result
   (non-admin requests draw down a rolling request quota, if the admin set one). Each result
   shows whether the book is already in your library, already on the way, or requestable.
...
6. **Track to available.** narratorr-requests polls narratorr and advances the request to
   `available` once narratorr finishes importing — or to **Failed**, with a plain-English
   reason. The requester watches this **live on their My Requests page** — it updates every few
   seconds.

</Steps>

:::note[Two kinds of notifications]
**Admin notifications** fire on a new request to review, a new user awaiting approval, and a
request that failed to acquire — over any notifiers you add (ntfy, email, Discord, Slack,
Telegram, Pushover, Gotify, or a generic webhook).

**Requester emails** are opt-in, per user: clicking your username opens the account modal, where
you set a contact email and choose which events to be emailed about — your request being
**approved**, **denied** (with the admin's reason, when given), or **ready to listen**. Delivery
needs two things — a saved contact email on your account and an **Email (SMTP)** notifier
configured by an admin; until both exist the opt-ins are shown disabled. Requesters can always
follow status live on their **My Requests** page either way.
:::

## Companion eBooks

If narratorr keeps a **companion EPUB** alongside an audiobook, narratorr-requests can offer that
too: library books grow a green **Get eBook** action — download the EPUB, or **Send to Kindle**
(the app emails it to the user's device, and walks them through Amazon's one-time approved-sender
setup). It's opt-in on both sides: an admin toggle in
[Settings](/narratorr-requests/configuration/#ebooks) plus eBook support enabled in narratorr
itself — and Send to Kindle additionally needs a configured
[Kindle sender](/narratorr-requests/configuration/#kindle-sender). See
[Using → eBooks](/narratorr-requests/using/#ebooks-download-or-send-to-kindle).
...
- [Using narratorr-requests](/narratorr-requests/using/) — search, request, grab companion
  eBooks, and manage the queue.
