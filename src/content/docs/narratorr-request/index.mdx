...
**narratorr-request** is an [Overseerr](https://overseerr.dev/)-style request manager for
...
narratorr-request is a **plug-in sidecar**. It talks to narratorr **only** over narratorr's
...
- The **API key is server-to-server only.** It lives in narratorr-request's backend and is
  never sent to the browser. The browser only ever calls narratorr-request's own `/api/*`.
- narratorr-request **polls** narratorr (`GET /books/:id`) to follow a request's progress — there
  is no webhook back from narratorr.
- You configure the narratorr connection (and notification channels) in narratorr-request's own
...
1. **Sign in.** A user signs in with local email/password or an OIDC provider (Plex bridge,
   Authelia, etc. — see [Authentication](/narratorr-request/authentication/)).
...
5. **Handoff to narratorr.** On approval, narratorr-request adds the book to narratorr (idempotent
   by ASIN) and the request enters `acquiring`.

6. **Track to available.** narratorr-request polls narratorr and advances the request to
...
narratorr-request notifies the **admin** of a new request to review and a new user awaiting
approval (over ntfy / email / webhook). It does **not** notify the requester when their book
becomes available — requesters follow status on their **My Requests** page. (A requester-facing
"your book is ready" notification is planned for a future release.)
:::
...
- [Installation](/narratorr-request/installation/) — run it with Docker.
- [Authentication](/narratorr-request/authentication/) — local and OIDC sign-in, the approval
  queue, and admin bootstrap.
- [Configuration](/narratorr-request/configuration/) — connect to narratorr and set up
  notifications in the Settings page.
- [Using narratorr-request](/narratorr-request/using/) — search, request, and manage the queue.
- [Troubleshooting](/narratorr-request/troubleshooting/) — common first-run issues.
